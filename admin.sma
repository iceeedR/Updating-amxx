...
// pcvars
new bool:amx_mode
new amx_password_field[MAX_NAME_LENGTH]
new amx_default_access[10]
new bool:amx_admins_mysql
...
public plugin_init()
{
	...
	bind_pcvar_num(create_cvar("amx_mode", "1", FCVAR_PROTECTED), amx_mode)
	bind_pcvar_num(create_cvar("amx_use_mysql", "0", FCVAR_PROTECTED), amx_admins_mysql)
	bind_pcvar_string(create_cvar("amx_password_field", "_pw", FCVAR_PROTECTED), amx_password_field, charsmax(amx_password_field))
	bind_pcvar_string(create_cvar("amx_default_access", "z", FCVAR_PROTECTED), amx_default_access, charsmax(amx_default_access))

	register_plugin(fmt("Admin Base%s", amx_admins_mysql ? " (SQL)" : ""), AMXX_VERSION_STR, "AMXX Dev Team")
	...
	create_cvar("amx_show_activity", "2", FCVAR_PROTECTED)
...
AddAdmin(id, auth[], accessflags[], password[], flags[], comment[]="")
{
	...
	if(amx_admins_mysql)
	{
		...
		sql = SQL_Connect(info, errno, error, charsmax(error))
...
getAccess(id, name[], authid[], ip[], password[])
{
	...
	else 
	{
		...
		if (!strlen(amx_default_access))
		{
			copy(amx_default_access, charsmax(amx_default_access), "z")
		}
		
		new idefaccess = read_flags(amx_default_access)
...
accessUser(id, name[] = "")
{
	...
	get_user_info(id, amx_password_field, password, charsmax(password))
