...
public plugin_init()
{
	...
	register_clcmd("say /msounds", "cmdSwitchSounds", _, " - muda os sons entre ativados e desativados")
	register_clcmd("say_team /msounds", "cmdSwitchSounds", _, " - muda os sons entre ativados e desativados")
}
	...
	if (KillingStreak)
	{
		...
		for (new a = 0; a < ppl; ++a)
		{
			...
			if (g_streakKills[i][0] >= 2)
				client_print_color(i, print_team_default, "^4*^3 %L", i, "KILLED_ROW", g_streakKills[i][0])
			else if (g_streakKills[i][1] >= 2)
				client_print_color(i, print_team_default, "^4*^3 %L", i, "DIED_ROUNDS", g_streakKills[i][1])
		}
	}
}
...
public bomb_planted(planter)
{
	...
	bombTimer()
	set_task_ex(1.0, "bombTimer", TASK_BOMB_TIMER, .flags = SetTask_Repeat)
}
...
public cmdSwitchSounds( id )
{
	...
	client_print_color(id, print_team_default, "^3MSounds^4 %L", id, g_msounds[id] ? "ativado" : "desativado")
